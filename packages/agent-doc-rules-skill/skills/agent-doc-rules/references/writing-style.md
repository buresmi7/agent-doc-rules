...
## Structured Style Review

When the user explicitly asks for a style or plain-English review, the current
host agent applies this reference directly. Do not start a second AI tool.

Inspect the requested files, or changed documentation when no files are named.
Report only findings that improve meaning, accuracy, or reader effort. Do not
create churn for a harmless preference.

Use this shape for each finding:

```text
<severity> path/to/file.md:line
Issue: <specific clarity, structure, terminology, or accuracy problem>
Replacement: <concrete wording or structural fix>
```

Use `error` when wording is misleading, contradictory, or hides an important
constraint. Use `warning` for material clarity or structure problems. Omit
minor preferences. If the user asked for edits, apply the fixes and run the
repository's deterministic documentation check afterward.
