...
5. Load only the references needed for the task:
   ...
   - For documentation writing, rewriting, or style review, read
     ...
     The current host agent applies that reference directly. For an explicit
     style or plain-English review, use its structured review format instead of
     starting another AI tool.
   ...
   - For deterministic documentation validation, read
     [references/validation.md](references/validation.md). For a corpus-wide
     semantic duplicate review, use `$docs-duplicate-review`. For
...
- When project notes describe a repeated workflow meant for agents, create or
  ...
  at its original location with a short Markdown link to the skill instead of
  storing a second copy there. A skill name or path in code formatting alone is
  not a link.
