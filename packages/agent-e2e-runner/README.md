...
the isolated project, Codex event logs, a portable `agent-session.json`, a
...
error includes available artifact paths in its message. Pass `outputRoot` to
place run directories outside the scenario.
